...
cdef class ArgmaxDecoder(DecoderBase):
      ...
      cpdef decode(self, double[::1,:] probs):
            ...
            for t in range(probs.shape[1]):
                ...
                if maxInd[t] != pmInd:  # collapsing duplicates
                    pmInd = maxInd[t]
                    if pmInd > -1 and pmInd != 29:  # 29: <blank>
...
cdef class BeamLMDecoder(DecoderBase):
      ...
      def lm_score_final_char(self, prefix, query_char):
            ...
            #print(query_char, prob_list[-2])
            #for i, (prob, length) in enumerate(full_scores):
                #print (words[i], length, prob)
            return prob_list[-2]


      def decode(self, double[::1,:] probs,
                   unsigned int beam=100, double alpha=1.25, double beta=1.5):
            """
            Decoder with an LM
            returns the best hypothesis in characters
            Charmap must be loaded
            ...
            for t in xrange(T):
                  ...
                  for prefix,(v0,v1,numC) in Hcurr.iteritems():
                        ...
                        valsP = Hnext[prefix]
                        valsP[1] = self.combine(v0+probs[bnk,t],v1+probs[bnk,t],valsP[1])
                        valsP[2] = numC
                        if len(prefix) > 0:
                              valsP[0] = self.combine(v0+probs[prefix[-1],t],valsP[0])

                        for i in xrange(0,N-1):  # blank: 0 --> N-1
                              ...
                              if nprefix not in Hcurr:
                                    ...
                                    valsN[1] = self.combine(v2+probs[bnk,t],v3+probs[bnk,t],valsN[1])
